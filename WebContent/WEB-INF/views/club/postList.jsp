...
<body>
	...
	<section style="width: 100%; height: 900px;">
		<h2 style="position: relative; top: 3%; left: 3%;">${clubName }
			게시판</h2>
		<hr style="position: relative; top: 3%; background-color: D3D3D3;">
		<div
			style="width: 100%; height: 80%; position: relative; top: 3%; text-align: center;">
			<table class="t__contents">
				...
				<c:forEach items="${cpList }" var="cpOne" varStatus="index">
					<tr align="center">
						<td>${cpOne.postNo }</td>
						<td><a href="/clubPost/detail?postNo=${cpOne.postNo }&clubNo=${cpOne.clubNo}">${cpOne.postSubject }</a></td>
						<td>${cpOne.userId }</td>
			...
			<!-- 여기서 serch작업 -->
			<form action="/clubPost/search" method="post"
				style="display: inline; position: relative; right: 31%;">
				<input type="text" placeholder="제목을 입력하세요" name="keyword"> <input
					type="submit" value="검색">
			</form>
			<form style="display: inline; position: relative; left: 31%;"
				action="/clubPost/write" method="get">
